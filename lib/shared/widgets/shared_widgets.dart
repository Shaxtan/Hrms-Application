  ...
  Widget build(BuildContext context) {
    ...
    return AppBar(
      ...
      leading: showMenuButton
          ? _MenuButton(t: t, onTap: onMenuTap)
          : null,
      title: Column(
  ...
  Widget build(BuildContext context) {
    ...
    switch (variant) {
      case AppButtonVariant.primary:
        ...
        fg = Colors.white; borderColor = null; break;
      case AppButtonVariant.secondary:
        bg = AppColors.accentLight; fg = AppColors.accent;
        borderColor = AppColors.accent.withOpacity(0.3); break;
      case AppButtonVariant.ghost:
        bg = Colors.transparent; fg = t.textSec; borderColor = t.border; break;
    ...
    final child = loading
        ...
        : Row(mainAxisSize: MainAxisSize.min,
            ...
            children: [
              ...
              Text(label, style: AppTextStyles.buttonMedium.copyWith(color: fg)),
            ]);
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        TextFormField(
          ...
          style: AppTextStyles.bodyMedium.copyWith(
              color: enabled ? t.textPrimary : t.textTert),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        DropdownButtonFormField<T>(
          ...
          hint: hint != null
              ? Text(hint!, style: AppTextStyles.bodyMedium.copyWith(color: t.textTert))
              : null,
          ...
          icon: Icon(Icons.keyboard_arrow_down_rounded, color: t.textSec, size: 20),
          isExpanded: true,
        ),
      ]);
  }
...
class SectionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t        = ThemeController.to;
    final topColor = hasError ? AppColors.danger : (accentColor ?? AppColors.accent);
    return Container(
        decoration: BoxDecoration(
          color: t.surface,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          AnimatedCrossFade(
            ...
            secondChild: Column(children: [
              ...
              Padding(padding: const EdgeInsets.all(AppSpacing.base), child: child),
            ]),
...
class StatusBadge extends StatelessWidget {
  ...
  (Color, String, Color) _config(String s) {
    switch (s.toUpperCase()) {
      case 'ACTIVE':               return (AppColors.successLight, 'Active',           AppColors.success);
      case 'PENDING_APPROVAL':     return (AppColors.warningLight, 'Pending Approval',  AppColors.warning);
      case 'PENDING_SALARY_SETUP': return (AppColors.infoLight,   'Pending Setup',     AppColors.info);
      case 'REJECTED':             return (AppColors.dangerLight,  'Rejected',          AppColors.danger);
      case 'UNASSIGNED':           return (const Color(0xFFE2E8F0), 'Unassigned',       AppColors.textSecondary);
      default:
        return (const Color(0xFFE2E8F0), _humanize(s), AppColors.textSecondary);
    }
  }

  String _humanize(String s) => s.toLowerCase().split('_')
  ...
  Widget build(BuildContext context) {
    ...
    return ClipRRect(
      ...
      child: SizedBox(height: 8, child: Row(children: [
        if (aFlex > 0) Expanded(flex: aFlex, child: Container(color: AppColors.pipelineApproved)),
        if (pFlex > 0) Expanded(flex: pFlex, child: Container(color: AppColors.pipelinePending)),
        if (rFlex > 0) Expanded(flex: rFlex, child: Container(color: AppColors.pipelineRejected)),
        if (rem > 0)   Expanded(flex: rem,   child: Container(color: t.surfaceVar)),
      ])),
    );
  }
}
    ...
    return Container(
        ...
        child: Row(children: [
          Container(width: 36, height: 36,
              decoration: BoxDecoration(
                  color: hasFile ? AppColors.success.withOpacity(0.15) : t.surface,
                  borderRadius: BorderRadius.circular(AppRadius.sm),
                  border: Border.all(color: t.border)),
              child: Icon(
                  hasFile ? Icons.check_circle_outline_rounded : Icons.upload_file_outlined,
                  color: hasFile ? AppColors.success : t.textSec, size: 18)),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (hasFile) ...[
              const SizedBox(height: 2),
              Text(fileName!, style: AppTextStyles.caption
                  .copyWith(color: AppColors.success),
                  maxLines: 1, overflow: TextOverflow.ellipsis),
            ] else if (helperText != null) ...[
              const SizedBox(height: 2),
              Text(helperText!, style: AppTextStyles.caption.copyWith(color: t.textTert)),
            ],
          ])),
          ...
          if (busy)
            ...
            GestureDetector(onTap: onPick,
                child: Container(
                    ...
                    child: Text('Upload', style: AppTextStyles.caption
...
class AadhaarCheckCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (state == 'ERROR') {
      return _card(AppColors.surfaceVariant, AppColors.textSecondary,
          Icons.wifi_off_rounded, 'Check unavailable',
          'Could not verify right now. Server will validate on submit.', []);
    }
    if (state == 'ACTIVE_DUPLICATE' || state == 'PENDING_DUPLICATE') {
      return _card(AppColors.dangerLight, AppColors.danger,
          Icons.warning_amber_rounded, 'Aadhaar already registered',
    ...
    if (state == 'REHIRE') {
      return _card(AppColors.warningLight, AppColors.warning,
          Icons.person_add_alt_1_rounded, 'Previous employee found',
...
class StepResultLedger extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Setup Status', style: AppTextStyles.headingSmall
            .copyWith(color: t.textPrimary)),
        ...
        if (requiredFailures.isNotEmpty) ...[
          const SizedBox(height: AppSpacing.md),
          Row(children: [
            Expanded(child: AppButton(label: 'Discard',
                variant: AppButtonVariant.ghost,
                onPressed: onDiscard, loading: discarding)),
            const SizedBox(width: AppSpacing.sm),
            Expanded(child: AppButton(label: 'Retry Upload',
                onPressed: onRetry, loading: retrying)),
          ]),
        ] else if (onContinue != null) ...[
          const SizedBox(height: AppSpacing.md),
          AppButton(label: 'Continue to Profile',
              onPressed: onContinue, fullWidth: true),
        ],
      ]),
    );
  }
  ...
  Widget _row(String key, dynamic val, ThemeController t) {
    ...
    switch (status) {
      ...
      case 'failed':  icon = Icons.cancel_rounded;                color = AppColors.danger;  break;
      case 'running': icon = Icons.pending_rounded;               color = AppColors.info;    break;
      case 'skipped': icon = Icons.remove_circle_outline_rounded; color = t.textTert;        break;
      default:        icon = Icons.radio_button_unchecked_rounded; color = t.textTert;
    }
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(width: AppSpacing.sm),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: AppTextStyles.bodySmall.copyWith(color: t.textPrimary)),
          if (error != null)
            Text(error, style: AppTextStyles.caption.copyWith(color: AppColors.danger)),
        ])),
      ]),
    );
  }

  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      return Scaffold(
        ...
        appBar: SharedAppBar(
          title: 'Employees',
          subtitle: '${ctrl.totalRecords.value} records',
          extraActions: [
            IconButton(
      ...
      child: Column(children: [
        TextField(
          onChanged: (v) {
            ...
            // Debounce: re-fetch from server after typing stops
            Future.delayed(const Duration(milliseconds: 400), () {
              if (ctrl.searchQuery.value == v) ctrl.fetchEmployees();
            });
          },
      ...
      return GestureDetector(
        onTap: () {
          if (type == 'status')
            ...
            ctrl.typeFilter.value = value;
          ctrl.fetchEmployees(); // Re-fetch with new filter
        },
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Padding(
              ...
              child: Row(children: [
                _EmployeeAvatar(emp: emp, size: 48),
                const SizedBox(width: 12),
                Expanded(
                    child: Column(
...
// ── Reusable avatar that shows photo or initials fallback ──────────────────
class _EmployeeAvatar extends StatelessWidget {
  final Employee emp;
  final double size;
  const _EmployeeAvatar({required this.emp, this.size = 48});

  @override
  Widget build(BuildContext context) {
    final hasPhoto =
        emp.profilePhotoUrl != null && emp.profilePhotoUrl!.isNotEmpty;
    // Use the API path (relative) so Dio's interceptor adds auth headers
    final photoPath =
        hasPhoto ? emp.profilePhotoUrl! : '/api/v1/employees/${emp.id}/photo';
    ...
    return Container(
      ...
      decoration:
          BoxDecoration(borderRadius: BorderRadius.circular(size * 0.3)),
      child: AuthImage(
        url: photoPath,
        width: size,
        height: size,
        ...
        child: Center(
            child: Text(emp.initials,
                style: AppTextStyles.headingSmall
                    .copyWith(color: Colors.white, fontSize: size * 0.28))),
      );
}
